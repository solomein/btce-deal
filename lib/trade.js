...
function BTCETrade (key, secret, host) {
    this.key = key;
    this.secret = secret;
...
BTCETrade.prototype.setNextNonce = function (nonce) {
  this.nonce = nonce*1;
}

BTCETrade.prototype.checkNonce = function () {
  if (this.lastNonce && (this.nonce <= this.lastNonce))
    throw new Error("Invalid BTCE nonce");
  this.lastNonce = this.nonce;
  console.log(' nonce type is ',typeof this.nonce, ' nonce is',this.nonce);
  return this.nonce;
}
